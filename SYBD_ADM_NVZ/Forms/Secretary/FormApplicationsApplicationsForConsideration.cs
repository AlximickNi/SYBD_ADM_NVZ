...
{
    public partial class FormApplicationsApplicationsForConsideration : Form
    {
...
where Id_Secretary = {FormMain.Id_Special_Worker} and Applications_Status = 'Ожидание'
...
where Id_Secretary = {FormMain.Id_Special_Worker} and W.Id_Worker = AFD.Id_Worker and Applications_Status = 'Ожидание'
order by Id_Application";
...
Продолжительностью с {Convert.ToDateTime(Res["Date_Start"]).ToShortDateString()} до {Convert.ToDateTime(Res["Date_End"]).ToShortDateString()} включительно.";
                ...
                Appli.Id_Appli = $@"{Res["Id_Application"]}";
                Appli.TypeTably_Appli = "Applications_Vacation_Or_Sick_Leave";
        ...
        private void ControlAppli_Click(object sender, EventArgs e)
        {
            if (CurrentItemAppli != (sender as Control).Parent as ItemAppli)
            {
                if (CurrentItemAppli != null)
                {
                    ...
                    CurrentItemAppli.LblStatus.ForeColor = Color.Black;
                    CurrentItemAppli.LblDateEnd.ForeColor = Color.Black;
        ...
        public FormApplicationsApplicationsForConsideration()
        {
            InitializeComponent();
        }

        private void FormApplicationsApplicationsForConsideration_Load(object sender, EventArgs e)
        {
            BtnAccept.BackColor = Color.FromArgb(46, 139, 87);
            BtnDismiss.BackColor = Color.FromArgb(128, 0, 0);
            FillPanelAppli();
        }
        ...
        private void BtnAccept_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Выберите заявку чтобы Принять или Отклонить её.", "Внимание!", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            if (MessageBox.Show("Вы уверены с решением?", "Внимание!", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                SqlConnection Con = new SqlConnection(FormMain.TxtCon);
                Con.Open();
                string TxtQuarty1 = "";
                if(Currrent_TypeTably_Appli == "Applications_Vacation_Or_Sick_Leave")
                {
                    TxtQuarty1 = $@"
update {Currrent_TypeTably_Appli}
set Applications_Status = 'Принято',
Date_of_the_final_decision_of_the_application = '{DateTime.Now.ToString("MM.dd.yyyy")}'
...
where Id_Application = {Currrent_Id_Appli}";
        ...
        private void BtnDismiss_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("Вы уверены с решением?", "Внимание!", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                TxtQuarty1 = $@"
update {Currrent_TypeTably_Appli}
set Applications_Status = 'Отклонено',
Date_of_the_final_decision_of_the_application = '{DateTime.Now.ToString("MM.dd.yyyy")}'
where Id_Application = {Currrent_Id_Appli}";
                SqlCommand Quarty1 = new SqlCommand(TxtQuarty1, Con);
                Quarty1.ExecuteNonQuery();
